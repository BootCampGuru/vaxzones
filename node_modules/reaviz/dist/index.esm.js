import React, { Component, PureComponent, useMemo, cloneElement, Fragment, createRef, Children, useState, useCallback, useEffect, useRef } from 'react';
...
const ChartContainer = (_a) => {
    ...
    return (React.createElement("div", { ref: ref, style: { height: styleHeight, width: styleWidth } }, height && width && (React.createElement("svg", { width: width, height: height, className: className, style: style },
...
const ScatterPoint = (_a) => {
    ...
    const transitionProps = useMemo(() => animated ? Object.assign(Object.assign({}, DEFAULT_TRANSITION), { delay: index * 0.005 }) : {
        type: false,
        delay: 0
    }, [index, animated]);
...
class ScatterPlot extends Component {
    ...
    render() {
        ...
        return (React.createElement(ChartContainer, { id: id, width: width, height: height, margins: margins, xAxisVisible: isAxisVisible(xAxis.props), yAxisVisible: isAxisVisible(yAxis.props), className: classNames(css$n.scatterPlot, className) }, props => this.renderChart(props)));
    }
}
...
class PointSeries extends Component {
    ...
    render() {
        ...
        return (React.createElement(ScatterSeries, { height: height, width: width, id: id, animated: animated, data: data, xScale: xScale, yScale: yScale, point: React.createElement(CloneElement, { element: point, color: color, className: css$p.point, size: 4, tooltip: null, visible: this.isVisible.bind(this) }) }));
    }
}
...
class AreaChart extends Component {
    ...
    render() {
        ...
        return (React.createElement(ChartContainer, { id: id, width: width, height: height, margins: margins, xAxisVisible: isAxisVisible(xAxis.props), yAxisVisible: isAxisVisible(yAxis.props), className: classNames(css$r.areaChart, className, series.type) }, props => this.renderChart(props)));
    }
}
...
class BarChart extends Component {
    ...
    render() {
        ...
        return (React.createElement(ChartContainer, { id: id, width: width, height: height, margins: margins, xAxisVisible: isAxisVisible(xAxis.props), yAxisVisible: isAxisVisible(yAxis.props), className: classNames(css$v.barChart, className, css$v[series.props.type]) }, props => this.renderChart(props)));
    }
}
...
class HiveNode extends Component {
    render() {
        ...
        return (React.createElement("circle", { className: classNames(css$x.node, {
...
class HiveAxis extends Component {
    render() {
        ...
        return (React.createElement(Fragment, null,
            React.createElement("line", { className: css$z.axis, style: { stroke: color(index) }, transform: `rotate(${getDegrees(angle(index))})`, x1: axisStart, x2: axisEnd }),
            React.createElement("line", { className: css$z.axis, style: { stroke: color(index) }, transform: `rotate(${getDegrees(angle(index)) + 90})`, x1: -axisLength / 20, x2: axisLength / 20, y1: -axisEnd, y2: -axisEnd }),
            React.createElement("line", { className: css$z.axis, style: { stroke: color(index) }, transform: `rotate(${getDegrees(angle(index)) + 90})`, x1: -axisStart / 3, x2: 0, y1: axisStart * -0.8, y2: axisStart * -1 }),
            React.createElement("line", { className: css$z.axis, style: { stroke: color(index) }, transform: `rotate(${getDegrees(angle(index)) + 90})`, x1: 0, x2: axisStart / 3, y1: -axisStart, y2: axisStart * -0.8 })));
    }
}
...
class HiveLabel extends Component {
    render() {
        ...
        return (React.createElement("text", { dy: translate(transform, outerRadius, label.padding), className: css$D.label, strokeWidth: "0.01", textAnchor: "middle", transform: `rotate(${transform})` }, text));
    }
}
...
class HiveTooltip extends Component {
    render() {
        ...
        return (React.createElement("div", null,
            React.createElement("div", { className: css$F.label },
                label,
                " - ",
                formatValue(value)),
            React.createElement("div", { className: css$F.value },
                formatValue(count),
                " Total")));
    }
}
...
class HivePlot extends Component {
    ...
    render() {
        return (React.createElement(ChartContainer, { height: this.props.height, width: this.props.width }, this.renderChart.bind(this)));
    }
}
...
const Map$1 = ({ id, width, height, margins, className, markers, data, fill = 'rgba(255, 255, 255, 0.3)' }) => {
    const getProjection = useCallback(({ chartWidth, chartHeight }) => geoMercator()
...
const MapMarker = ({ size = 3, index, tooltip, cx, cy, onClick = () => undefined }) => {
    ...
    return (React.createElement(Fragment, null,
        ...
        tooltip && (React.createElement(Tooltip, { visible: active, reference: ref, modifiers: modifiers, content: tooltip }))));
};

const PieArc = ({ color, data, innerArc, cursor = 'initial', animated = true, disabled = false, onClick = () => undefined, onMouseEnter = () => undefined, onMouseLeave = () => undefined, tooltip = React.createElement(ChartTooltip, null) }) => {
    ...
    const transition = useMemo(() => animated ? Object.assign({}, DEFAULT_TRANSITION) :
        {
            delay: 0
        }, [animated]);
    ...
    return (React.createElement("g", { ref: arcRef },
        React.createElement(motion.path, { transition: transition, d: d, style: { cursor }, fill: fill, onMouseEnter: event => {
                if (!disabled) {
                    setActive(true);
                    onMouseEnter({
                        value: data.data,
...
class SankeyLabel extends Component {
    render() {
        ...
        return (visible &&
            node && (React.createElement("text", { className: classNames(css$J.label, className), x: showRightSide ? nodePositions.x1 + 6 : nodePositions.x0 - 6, y: (nodePositions.y1 + nodePositions.y0) / 2, dy: "0.35em", textAnchor: textAnchor, fill: fill, opacity: opacity(active), style: { padding } }, node.title)));
    }
}
...
class SankeyLink extends Component {
    ...
    renderLink() {
        ...
        return (React.createElement("g", { ref: this.link },
            React.createElement(motion.path, { key: `sankey-link-${enterProps.d}-${index}`, className: classNames(css$L.link, className), style: style, initial: exitProps, animate: enterProps, exit: exitProps, transition: {
                    duration: 0.5
    ...
    renderTooltipContent() {
        ...
        return (React.createElement("div", { className: css$L.tooltip },
            React.createElement("div", { className: css$L.tooltipLabel }, `${source.title} → ${target.title}`),
            React.createElement("div", { className: css$L.tooltipValue }, formatValue(value))));
    }
    render() {
...
class SankeyNode extends Component {
    ...
    renderNode() {
        ...
        return (React.createElement(motion.g, { ref: this.rect, x: x0, y: y0 },
            React.createElement(motion.rect, { key: `sankey-node-${x0}-${x1}-${y0}-${y1}-${index}`, className: classNames(css$N.node, className), fillOpacity: opacity(active, disabled), style: style, width: nodeWidth, height: nodeHeight, fill: color, initial: {
                    opacity: 0
                }, animate: {
                    opacity: 1
                }, exit: {
    ...
    renderTooltipContent() {
        ...
        return (React.createElement("div", { className: css$N.tooltip },
            React.createElement("div", { className: css$N.tooltipLabel }, title),
            React.createElement("div", { className: css$N.tooltipValue }, formatValue(value))));
    }
    render() {
...
class RadialGaugeLabel extends PureComponent {
    render() {
        ...
        return (React.createElement("text", { dy: "1.23em", x: "0", y: offset, textAnchor: "middle", onClick: nativeEvent => onClick({ data, nativeEvent }), className: classNames(className, css$R.valueLabel) }, label));
    }
}
...
class RadialGaugeValueLabel extends PureComponent {
    render() {
        ...
        return (React.createElement(CountUp, { start: 0, end: data.data, delay: 0, duration: 1, separator: "," }, ({ countUpRef }) => (React.createElement("text", { dy: "-0.5em", x: "0", y: "15", textAnchor: "middle", className: classNames(className, css$T.valueLabel), ref: countUpRef }))));
    }
}
...
class HeatmapCell extends Component {
    ...
    render() {
        ...
        const transition = this.getTransition();
        return (React.createElement(Fragment, null,
            React.createElement(motion.g, { ref: this.rect, x: x, y: y },
                React.createElement(motion.rect, Object.assign({}, rest, { fill: fill, stroke: stroke, style: Object.assign(Object.assign({}, extras.style), { cursor }), className: classNames(css$V.cell, extras.className), initial: {
                        opacity: 0
                    }, animate: {
                        opacity: 1
                    }, exit: {
